...
// Initialize state with values from plugin settings
...
// Initialize from plugin settings on mount
onMount(() => {
...
function updateLineChart(projects: Project[], start: number, end: number) {
	...
	plugin.timesheetData.records.forEach((record) => {
		if (record.endTime < start || record.startTime > end) return;
		if (!projects.some((p) => p.id === record.projectId)) return;
		...
		if (days[key]) {
			...
			days[key][record.projectId] =
				(days[key][record.projectId] || 0) + duration;
		}
	});
	...
	const datasets = projects.map((project) => ({
		label: project.name,
		data: labels.map((label) => days[label][project.id] / 1000 / 60 / 60),
...
let totalTime = $derived.by(() => {
	...
	return projects.reduce((total, project) => {
		return (
			total +
			getProjectDuration(project.id, plugin.timesheetData.records, start, end)
		);
	}, 0);
});
